/* ============================================
   不動産投資 Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('realestate-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('realestate-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('realestate-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('realestate-progress', JSON.stringify(this.progress));
        localStorage.setItem('realestate-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        var html = '<div class="fade-in">' +
            ...
            '<h2>不動産投資 Academy へようこそ</h2>' +
            '<p>物件選定から融資・管理運営まで、不動産投資の全プロセスを体系的に学べるプラットフォームです。</p>' +
            '</div>' +
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('realestate-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('realestate-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('realestate-progress');
            localStorage.removeItem('realestate-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
